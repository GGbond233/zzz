...
bigname = "/root/data/modelshub/vicuna-7b-v1.3"
...
def build_dataset_rank(
    ...
    raw_data = []
    with open("ShareGPT_V4.3_unfiltered_cleaned_split.json", "r", encoding="utf-8") as f:
        for line in f:
            try:
    ...
    ds = Dataset.from_dict({
        "id": [item.get("id", str(i)) for i, item in enumerate(raw_data)],
        "conversations": [item.get("conversations", []) for item in raw_data]
    })
    # ds = ds['train']
